...
models_summary$elasticnet = Elastic_Cv(cv.sets = month.fit.val.k4.sets,
                                             my.lambda.vals = exp(seq(-20, -8, 0.1)) %>% sort(decreasing = TRUE),
                                             my.alpha.vals = seq(1e-5, 1 - 1e-5, length = 20))
```
...
```{r, eval = bool_execute_heavy_chunks}
models_summary$scad.noint = NonConvex_Cv(cv.sets = month.noint.fit.val.k4.sets,
...
models_summary$scad = NonConvex_Cv(cv.sets = month.fit.val.k4.sets,
                                             my.lambda.vals = exp(seq(-15, 1, 0.2)) %>% sort(decreasing = TRUE),
                                             my.gamma.vals = seq(2+1e-05, 20, length = 20),
                                         my.penalty = "SCAD")
...
TwoParErrPlot(model_list = models_summary$scad,
              ...
              row_par_min_name = "lmin",
              col_par_min_name = "gmin",
              my.main = "SCAD interaction Cv error contour",
              my.xlab = "log lambda",
              my.ylab = "gamma")
```
...
#### No interaction
```{r, eval = bool_execute_heavy_chunks}
models_summary$mcp.noint = NonConvex_Cv(cv.sets = month.noint.fit.val.k4.sets,
                                             my.lambda.vals = exp(seq(-15, 1, 0.2)) %>% sort(decreasing = TRUE),
...
#### Yes interaction
```{r, warning = FALSE, eval = bool_execute_heavy_chunks}
models_summary$mcp = NonConvex_Cv(cv.sets = month.fit.val.k4.sets,
                                             my.lambda.vals = exp(seq(-15, 1, 0.2)) %>% sort(decreasing = TRUE),
                                             my.gamma.vals = seq(1+1e-05, 20, length = 20),
                                         my.penalty = "MCP")
```
