...
year_grouped_zeros <- my.df %>%
  ...
  ungroup() %>%
  complete(KY_CD, LAW_CAT_CD, fill = list(count = 0))
```
...
```{r}
lambda.vals.poi = exp(seq(-20, -6, 0.1))

all.fit = glmnet(x = sparse.model.matrix(FORMULA.POI.NO.KY_CD,
                                         data = my.df.year.grouped),
                 y = my.df.year.grouped$count,
                 family = "poisson",
                 offset = log(my.df.year.grouped$Pop1),
                 alpha = 1)
...
for (i in 1:dim(lasso.cv.error.matr)[2]){
  temp.lasso = glmnet(x = month.fit.val.k4.sets[[i]]$train$model_matrix,
                 y = month.fit.val.k4.sets[[i]]$train$y,
                 family = "poisson",
                 offset = log(month.fit.val.k4.sets[[i]]$train$model_matrix),
                 alpha = 1,
                 lambda = lambda.vals)
