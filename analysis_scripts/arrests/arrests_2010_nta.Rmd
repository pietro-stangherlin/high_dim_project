...
for (i in 1:nrow(month_sets_ind$k4)){
  ...
  month.fit.val.k4.sets[[i]]$train$df = temp.df.train
  month.fit.val.k4.sets[[i]]$train$y = temp.df.train$y
  month.fit.val.k4.sets[[i]]$train$count = temp.df.train$count
  month.fit.val.k4.sets[[i]]$train$Pop1 = temp.df.train$Pop1
  month.fit.val.k4.sets[[i]]$train$model_matrix = sparse.model.matrix(FORMULA.NO.KY_CD.matrix,
                          data = temp.df.train)
  
  month.fit.val.k4.sets[[i]]$test$df = temp.df.test
  month.fit.val.k4.sets[[i]]$test$y = temp.df.test$y
  month.fit.val.k4.sets[[i]]$test$count = temp.df.test$count
  month.fit.val.k4.sets[[i]]$test$Pop1 = temp.df.test$Pop1
...
lambda.vals = exp(seq(-20, -6, 0.1))

all.fit = glmnet(x = sparse.model.matrix(FORMULA.NO.KY_CD,
...
for (i in 1:dim(lasso.cv.error.matr)[2]){
  ...
  lasso.cv.error.matr[,i] = apply(pred.lasso,
                                  2,
                                  function(col) RMSEfun(true_vals = exp(month.fit.val.k4.sets[[i]]$test$y),
                                                        pred_vals = exp(col)))
...
lasso.lambda.errs = cbind(lambda.vals %>% sort(decreasing = T),
...
```{r}
lasso.beta.best = all.fit$beta[,which.min(lasso.lambda.errs[,2])]
length(which(lasso.beta.best != 0)) / length(lasso.beta.best)
```
So the solution is not sparse even thought many coefficients are close to zero.

While for the l1se
```{r}
lasso.beta.1se = all.fit$beta[,lasso.1se.index]
length(which(lasso.beta.1se != 0)) / length(lasso.beta.best)
```
Still not sparse.

```{r}
lasso.beta.best %>% sort() %>% plot(main = "Lasso lambda best coefficients",
                                    xlab = "beta index",
                                    ylab = "beta value")
...
```{r}
alpha.vals = seq(1e-5, 1 - 1e-5, length = 20)
...
```{r}
# log(Pop1) is the offset
FORMULA.POI.NO.KY_CD = as.formula("count ~. -1 -Pop1 - y + 
                            NTA2020:LAW_CAT_CD +
                            NTA2020:AGE_GROUP +
                            NTA2020:PERP_SEX +
                            NTA2020:PERP_RACE")

Pop1.col.index = which(colnames(month.fit.val.k4.sets[[1]]$train$model_matrix) == "Pop1")
```
...
lambda.vals.poi = all.fit.poi$lambda # convergence issues for 36th lambda
