...
FORMULA.NO.INTERACTIONS = as.formula("y ~. -1 -Pop1 - count")
```
...
# NO Interaction
month.noint.fit.val.k4.sets = list()

for (i in 1:nrow(month_sets_ind$k4)){
  # make sublists
  month.noint.fit.val.k4.sets[[i]] = list(train = list(y = NA,model_matrix = NA),
                                    test = list(y = NA,model_matrix = NA))
  
  # populate the sublists
  # keep also the all df
  month.noint.fit.val.k4.sets[[i]]$train$df = GroupByMONTHSets(mydf = my.df,
                       month_indexes = setdiff(1:12, month_sets_ind$k4[i,]))
  
  month.noint.fit.val.k4.sets[[i]]$train$model_matrix = sparse.model.matrix(FORMULA.NO.INTERACTIONS,
                          data = month.fit.val.k4.sets[[i]]$train$df)
  
  
  month.noint.fit.val.k4.sets[[i]]$test$df = GroupByMONTHSets(mydf = my.df,
                       month_indexes = month_sets_ind$k4[i,])
  
  month.noint.fit.val.k4.sets[[i]]$test$model_matrix = sparse.model.matrix(FORMULA.NO.INTERACTIONS,
...
Lasso_CV <- function(cv_sets,
  ...
  temp_err_matr <- matrix(NA, 
                          nrow = length(my.lambda_vals),
                          ncol = length(cv_sets))
  
  for (i in 1:dim(temp_err_matr)[2]) {
    temp_fit <- glmnet(x = cv_sets[[i]]$train$model_matrix,
                       y = cv_sets[[i]]$train$df$y,
                       alpha = 1,
                       lambda = my.lambda_vals)
    
    pred_fit <- predict(temp_fit,
                        newx = cv_sets[[i]]$test$model_matrix)
    
    temp_err_matr[, i] <- apply(pred_fit, 2,
                                function(col) RMSEfun(true_vals = exp(cv_sets[[i]]$test$df$y),
                                                      pred_vals = exp(col)))
  ...
  cv_err_matr <- cbind(apply(temp_err_matr, 1, mean),
                                    apply(temp_err_matr, 1, sd) / ncol(temp_err_matr)) %>% as.data.frame()
  ...
  list_summary <- list()
  list_summary[["lambda"]] <- my.lambda_vals
  
  list_summary[["lmin"]] <- my.lambda_vals[lmin_index]
  list_summary[["l1se"]] <- my.lambda_vals[l1se_index]
...
```{r}
models_summary$lasso.noint <- Lasso_CV(month.noint.fit.val.k4.sets,
                                        my.lambda_vals = exp(seq(-17, -6, 0.1)) %>% sort(decreasing = T))
```
...
```{r}
models_summary$lasso <- Lasso_CV(month.fit.val.k4.sets,
                                        my.lambda_vals = exp(seq(-13, -6, 0.1)) %>% sort(decreasing = T))
```
...
Elastic_Cv <- function(cv.sets,
  ...
  for (k in 1:length(cv.sets)) {
    ...
    temp.err.array[,j,k] <- apply(pred_fit, 2,
                                function(col) RMSEfun(true_vals = exp(cv.sets[[k]]$test$df$y),
                                                      pred_vals = exp(col)))
  ...
  # rows: lambda
  # cols: alpha
  cv.err.matr <- cbind(apply(temp.err.array, c(1,2), mean),
                                    apply(temp.err.array, 1, sd) / ncol(temp.err.array)) %>% as.data.frame()
...
```{r}
models_summary$elasticnet.noint = Elastic_Cv(cv.sets = month.noint.fit.val.k4.sets,
                                             my.lambda.vals = exp(seq(-13, -8, 0.1)) %>% sort(decreasing = TRUE),
...
filled.contour(x = log(models_summary$elasticnet.noint$lambda) %>% sort(),
               y = models_summary$elasticnet.noint$alpha,
               z = models_summary$elasticne.noint$cv.err.matrix[(1:length(models_summary$elasticnet.noint$lambda)) %>% sort(decreasing = TRUE),],
               main = "Elasticnet no interaction Cv error contour",
               xlab = "log lambda",
               ylab = "alpha",
               ...
               plot.axes = { 
                 axis(1)
                 axis(2)
                 points(log(models_summary$elasticnet.noint$lmin), models_summary$elasticnet.noint$amin,
...
models_summary$elasticnet$lambda_vals =  exp(seq(-13, -8, 0.1)) %>% sort(decreasing = TRUE) # glmnet sorts by decreasing order
...
```{r}
# sort rows in z by increasing order to match that of lambda_vals (as required by filled.contours)
filled.contour(x = log(models_summary$elasticnet$lambda_vals) %>% sort(),
               y = models_summary$elasticnet$alpha_vals,
               z = models_summary$elasticnet$cv.err.matrix[(1:nrow(models_summary$elasticnet$cv.err.matrix)) %>% sort(decreasing = TRUE),],
               ...
               plot.axes = { 
                 axis(1)
                 axis(2)
                 points(log(models_summary$elasticnet$lmin), models_summary$elasticnet$amin,
                            col = "red", pch = 19, cex = 1.5)})
...
# the matrix rows order has to be inverted as well
filled.contour(x = log(models_summary$scad$lambda_vals) %>% sort(), # already sorted above
               y = models_summary$scad$gamma_vals,
               z = models_summary$scad$cv.err.matrix[(1:nrow(models_summary$scad$cv.err.matrix)) %>% sort(decreasing = TRUE),],
               ...
               plot.axes = { 
                 axis(1)
                 axis(2)
                 points(log(models_summary$scad$lmin), models_summary$scad$gmin,
                            col = "red", pch = 19, cex = 1.5)})
...
scad.fit.all = ncvreg(X = as.matrix(my.df.year.grouped.matrix),
                 ...
                 lambda = models_summary$scad$lambda_vals,
                 penalty = "SCAD")
...
```{r}
filled.contour(x = log(models_summary$mcp$lambda_vals) %>% sort(),
               y = models_summary$mcp$gamma_vals,
               z = models_summary$mcp$cv.err.matrix[(1:nrow(models_summary$mcp$cv.err.matrix)) %>% sort(decreasing = TRUE),],
               ...
               nlevels = 200,
                plot.axes = { 
                 axis(1)
                 axis(2)
                 points(log(models_summary$mcp$lmin), models_summary$mcp$gmin,
                            col = "red", pch = 19, cex = 1.5)})
...
legend("topleft",
       ...
       bty = "n")
```
The minimum is on the regulation parameters border, in particular $\gamma$ is close to one: the optimal solution seems not that reliable.
...
models_summary$lasso.poi = list()
models_summary$lasso.poi$lambda_vals = exp(seq(-15, 3, 0.1)) %>% sort(decreasing = TRUE)

temp.err.matr = matrix(NA, 
                      nrow = length(models_summary$lasso.poi$lambda_vals),
...
for (i in 1:dim(temp.err.matr)[2]){
  temp.fit = glmnet(x = month.fit.val.k4.sets[[i]]$train$model_matrix,
                 ...
                 offset = log(month.fit.val.k4.sets[[i]]$train$df$Pop1) + 
                                log(n_months_train),
                 alpha = 1,
                 lambda = models_summary$lasso.poi$lambda_vals,
                 intercept = TRUE,
                 standardize = TRUE)
  
  pred.fit = predict.glmnet(temp.lasso,
...
# compute error matrix and lambda min and 1se
models_summary$lasso$cv.err.matr = cbind(models_summary$lasso.poi$lambda_vals,
                          apply(temp.err.matr,1,mean),
                          apply(temp.err.matr,1,sd)/ncol(temp.err.matr)) %>% as.data.frame()

colnames(models_summary$lasso.poi$cv.err.matr) = c("lambda", "cv.err", "cv.se")

models_summary$lasso.poi$lmin.index = which.min(models_summary$lasso.poi$cv.err.matr$cv.err)

models_summary$lasso.poi$l1se.index = which.max(models_summary$lasso.poi$cv.err.matr$cv.err <=
                                              (models_summary$lasso.poi$cv.err.matr$cv.err[models_summary$lasso.poi$lmin.index] +
                                              models_summary$lasso.poi$cv.err.matr$cv.se[models_summary$lasso.poi$lmin.index]))

models_summary$lasso.poi$lmin = models_summary$lasso.poi$cv.err.matr$lambda[models_summary$lasso.poi$lmin.index]
models_summary$lasso.poi$l1se = models_summary$lasso.poi$cv.err.matr$lambda[models_summary$lasso.poi$l1se.index]
```


```{r}
PlotOneDim(x = log(models_summary$lasso.poi$cv.err.matr$lambda),
