...
Sia per SCAD che per MCP il $\lambda$ selezionato è prossimo a zero, provando ad aumentare ulteriormente $\gamma$ la soluzione sostanzialmente non cambia.
```{r, fig.show='hold', out.width = OUT.WIDTH, out.height= OUT.HEIGHT}
...
Per i modelli di Poisson adattati con conteggi nulli il $\lambda$ ottimi non tendono al metodo non penalizzato; nel caso di Elasticnet è selezionata una ridge.
```{r, fig.show='hold', out.width = OUT.WIDTH, out.height= OUT.HEIGHT}
par(mfrow = c(1,2))
...
Si riporta il grafico dei coefficienti per le categorie più granulari.
```{r}
