...
Senza considerare interazioni tra KY_CD (esplicativa non spaziale di arrest con più modalità) con gli NTA il rapporto è:
```{r, include = TRUE}
...
```{r, warning=FALSE, eval = bool_execute_heavy_chunks}
# models_summary$lasso.negbin = LASSO_NegBin_Offset_Cv(cv.sets = month.cv.k4.sets,
#                                                         my.lambda.vals = exp(seq(-10, 2, 0.2)) %>% sort(decreasing = TRUE),
...
models_summary$lasso$test_error = RMSEfun(df_2011_grouped_subsampled$count, 
                                          exp(predict(temp.fit, newx = df_2011_grouped_subsampled.matrix) + log(12)))
...
models_summary$scad$test_error = RMSEfun(df_2011_grouped_subsampled$count, 
                                          exp(my.predict.gaussian(temp.fit, newdata = df_2011_grouped_subsampled.matrix,
                                                               lambda.idx = 1) +
                                                log(12)))
...
Le stimne non sono sparse nemmeno con il criterio dell'errore a un errore standard
```{r,  fig.show='hold', out.width = OUT.WIDTH.1d, out.height= OUT.HEIGHT.1d}

par(mfrow = c(1,2))
plot(models_summary$lasso$beta1se, pch = 16,
     main = "LASSO 1se",
     xlab = "beta index", ylab = "beta")

plot(models_summary$lasso.poi$beta1se, pch = 16,
      main = "Poisson LASSO 1se",
     xlab = "beta index", ylab = "beta")

```


Anche per SCAD ed MCP non avviene selezione di variabili.
...
legend("bottomright",
       ...
       bty = "n")
```
Per provare ad avere qualche informazione sul fenomeno spiegato dai modelli si decide di selezionare un certo numero tra i coefficienti più grandi in valore assoluto.

```{r}
beta_names = df_2010_grouped_subsampled.matrix %>% colnames
```


```{r}
PlotFirstCoefsAbs(coef_vec = models_summary$elasticnet$beta,
                  coef_names = beta_names,
                  first_n = 20,
                  my.main = "Elasticnet biggest abs beta")

PlotFirstCoefsAbs(coef_vec = models_summary$scad$beta,
                  coef_names = beta_names,
                  first_n = 20,
                  my.main = "SCAD biggest abs beta")

PlotFirstCoefsAbs(coef_vec = models_summary$elasticnet.poi$beta,
                  coef_names = beta_names,
                  first_n = 20,
                  my.main = "Elasticnet poi biggest abs beta")
```
