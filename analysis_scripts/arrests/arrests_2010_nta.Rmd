...
Lasso_Offset_CV <- function(cv_sets,
  ...
  for (i in 1:length(cv_sets)) {
    temp_fit <- glmnet(x = cv_sets[[i]]$train$model_matrix,
                       ...
                       lambda = my.lambda.vals,
                       family = my.family,
                       offset = log(my.offset_constant_train) + log(cv_sets[[i]]$train$df[[my.offset_var_name]]))
    
    pred_fit <- predict(temp_fit,
                        newx = cv_sets[[i]]$test$model_matrix,
                        newoffset = log(my.offset_constant_test) + log(cv_sets[[i]]$test$df[[my.offset_var_name]]),
                        type = "response")
...
```{r, eval = bool_execute_heavy_chunks}
models_summary$lasso.poi.noint = Lasso_Offset_CV(cv_sets = month.noint.fit.val.k4.sets,
...
#### Yes interaction
```{r, eval = bool_execute_heavy_chunks}
models_summary$lasso.poi = Lasso_Offset_CV(cv_sets = month.fit.val.k4.sets,
                                          my.lambda.vals = exp(seq(-15, 3, 0.1)) %>% sort(decreasing = TRUE),
                                          my.response.name = "count",
                                          my.family = poisson(),
                                          my.offset_constant_train = 9, # 9 months
...
Elastic_Offset_Cv <- function(cv.sets,
  ...
  for (k in 1:length(cv.sets)) {
    ...
    temp_fit <- glmnet(x = cv.sets[[k]]$train$model_matrix,
                       ...
                       lambda = my.lambda.vals,
                       family = my.family,
                       offset = log(my.offset_constant_train) + log(cv.sets[[k]]$train$df[[my.offset_var_name]]))
    
    pred_fit <- predict(temp_fit,
                        newx = cv.sets[[k]]$test$model_matrix,
                        newoffset = log(my.offset_constant_test) + log(cv.sets[[k]]$test$df[[my.offset_var_name]]))
...
```{r, warning=FALSE, eval = bool_execute_heavy_chunks}
models_summary$elasticnet.poi.noint = Elastic_Offset_Cv(cv.sets = month.noint.fit.val.k4.sets,
...
```{r, warning=FALSE, eval = bool_execute_heavy_chunks}
models_summary$elasticnet.poi = Elastic_Offset_Cv(cv.sets = month.fit.val.k4.sets,
                                                        my.lambda.vals = exp(seq(-10, 2, 0.2)) %>% sort(decreasing = TRUE),
                                                        my.alpha.vals = seq(1e-5, 1 - 1e-5, length = 20),
                                                        my.response.name = "count",
                                                        my.family = poisson(),
...
```{r, warning=FALSE, eval = bool_execute_heavy_chunks}
models_summary$lasso.negbin.noint = LASSO_NegBin_Offset_Cv(cv.sets = month.noint.fit.val.k4.sets,
...
#### Yes interaction
```{r, warning=FALSE, eval = bool_execute_heavy_chunks}
models_summary$lasso.negbin = LASSO_NegBin_Offset_Cv(cv.sets = month.fit.val.k4.sets,
                                                        my.lambda.vals = exp(seq(-10, 2, 0.2)) %>% sort(decreasing = TRUE),
